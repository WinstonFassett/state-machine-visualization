(function (factory) {
	...
	var hasRequiredStateForceGraph;

	function requireStateForceGraph () {
		if (hasRequiredStateForceGraph) return StateForceGraph;
		hasRequiredStateForceGraph = 1;
		...
		class StateForceGraphComponent extends HTMLElement {
		    ...
		    switch (name) {
		      ...
		      case 'last-event':
		        ...
		        if (this.graph && this._lastEvent) {
		          const link = this.graph.graphData().links.find(it => it.source.id === this._prevState && it.name === this._lastEvent.type);
		          if (link) {
		  ...
		  updateGraphData() {
		    ...
		    const {
		      nodes,
		      links
		    } = this.createDiagram(this.definition);
		  ...
		  createDiagram(definition) {
		    ...
		    stateIds.forEach(key => {
		      ...
		      state.on && Object.keys(state.on).forEach(name => {
		    ...
		    links.forEach(link => {
		      link.nodePairId = link.source.id <= link.target.id ? link.source.id + "_" + link.target.id : link.target.id + "_" + link.source.id;
    ...
    `;
		    ...
		    const Graph = commonjsGlobal.ForceGraph()(container);
		    this.graph = Graph;
		    ...
		    Graph.height(300).linkCurvature("curvature").linkDirectionalArrowLength(6).linkDirectionalArrowRelPos(1).nodeCanvasObjectMode(() => "after").nodeCanvasObject((node, ctx, globalScale) => {
		    ...
		    }).nodeId("id").nodeLabel("name").nodeAutoColorBy("name").linkCanvasObjectMode(() => "after").linkCanvasObject((link, ctx) => {
		      ...
		      if (+link.curvature > 0) {
		        if (link.__controlPoints) textPos = getQuadraticXY(0.5, start.x, start.y, link.__controlPoints[0], link.__controlPoints[1], end.x, end.y);
		      }
		      const relLink = {
		        x: end.x - start.x,
		        y: end.y - start.y
		      ...
		      const color = this.value === link.source.name && canFire(this.definition, this.value, link.name) ? activeLinkColor : linkColor;
		      ...
		      if (this._prevState === link.source.name && this._lastEvent && this._lastEvent.type === link.name) {
		        return transitionColor;
		      }
		      return this.value === link.source.name && canFire(this.definition, this.value, link.name) ? activeLinkColor : linkColor;
		    }).linkDirectionalParticleColor(() => particleColor).linkDirectionalParticleSpeed(0.04).linkDirectionalParticleWidth(8).linkHoverPrecision(10).onLinkClick(({
		      name
		    }) => {
		...
		return StateForceGraph;
	}

	requireStateForceGraph();
